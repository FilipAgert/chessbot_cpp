...
TEST(magic_test, rookmagic) {
    ...
    for (int i = 0; i < 64; i++) {
        ...
        std::array<uint64_t, max_size> atk = compute_atk_bbs(occvar, i, true, false);
        int nelems = 1 << m;
...
TEST(Movegentest, mated) {
    ...
    ASSERT_EQ(expected, num_moves) << "Expected number of moves is 1. Actual found moves is" << num_moves;
}
...
TEST(Movegentest, promo_BUG) {
    ...
    ASSERT_EQ(expected, num_moves) << "Expected number of moves is 41. Actual found moves is" << num_moves;
}
...
TEST(perft, kiwipete) {
    ...
    std::string starting_fen = "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 1 1";
    int max = 4;
    for (int i = 0; i < max; i++) {
...
TEST(perft, p6) {
    ...
    std::string starting_fen = "r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10";
    int max = 4;
    for (int i = 0; i < max; i++) {
        int num_moves = movegen_benchmark::gen_num_moves(starting_fen, i + 1);
        size_t expected = moves[i];
...
TEST(Movegentest, capture_check) {
    ...
    std::vector<std::string> expected_moves = {// Castling: NONE allowed because King is in check.
...
TEST(Movegentest, two_deep) {
    ...
    expected = 20 * 20;
    ASSERT_EQ(expected, num_moves) << "Expected number of moves is 40. Actual found moves is" << num_moves;
}
TEST(bbgentest, white_pawns) {
    uint64_t w_pawn_bb = BitBoard::one_high(idx_from_string("a2"));
    uint64_t expected = BitBoard::one_high(idx_from_string("a3")) | BitBoard::one_high(idx_from_string("a4"));
...
TEST(bbgentest, black_pawns) {
    ...
    uint64_t expected = BitBoard::one_high(idx_from_string("a6")) | BitBoard::one_high(idx_from_string("a5"));
...
TEST(bbgentest, fen1) {
    ...
    std::array<Move, max_legal_moves> moves;
    size_t num_moves = board.get_moves<normal_search, false>(moves);
    std::vector<std::string> expected_moves = {"a7a6", "b7b6", "c7c6", "d7d6", "e7e6", "f7f6", "g7g6", "h7h6", "a7a5", "b7b5",
...
TEST(castle_test, Disallowed_White_NoFlag) {
    ...
    std::vector<std::string> expected_moves = {// King moves (e1)
...
TEST(castle_test, Allowed_White_Kingside_Queenside) {
    ...
    std::vector<std::string> expected_moves = {         // Castling Moves
...
TEST(castle_test, Disallowed_White_Kingside_Blocked) {
    ...
    std::vector<std::string> expected_moves = {// Castling: Only Queenside allowed (e1c1)
                                               "e1c1",
...
TEST(castle_test, Disallowed_Black_Kingside_Blocked) {
    ...
    std::vector<std::string> expected_moves = {// Castling: Only Queenside allowed
                                               "e8c8",
                                               // King Moves
...
TEST(castle_test, Disallowed_Black_Queenside_Blocked) {
    ...
    std::vector<std::string> expected_moves = {// Castling: Only Kingside allowed
                                               "e8g8",
                                               // King Moves
                                               "e8d8", "e8f8", "e8d7", "e8e7", "e8f7",
                                               // Rook h8 (Full range)
...
TEST(castle_test, Disallowed_White_bothsides_ThruCheck1) {
    ...
    std::vector<std::string> expected_moves = {// King Moves (d1 is attacked, so King cannot move there normally either)
                                               "e1d2", "e1e2", "e1f2",
                                               // Rook a1 (Full)
                                               "a1b1", "a1c1", "a1d1", "a1a2", "a1a3", "a1a4", "a1a5", "a1a6", "a1a7", "a1a8",
                                               // Rook h1 (Full)
...
TEST(castle_test, Disallowed_White_bothsides_ThruCheck2) {
    ...
    std::vector<std::string> expected_moves = {// King moves
                                               "e1e2", "e1d1", "e1f1",
                                               // Rook a1 (Full)
                                               "a1b1", "a1c1", "a1d1", "a1a2", "a1a3", "a1a4", "a1a5", "a1a6", "a1a7", "a1a8",
                                               // Rook h1 (Full)
...
TEST(castle_test, Disallowed_Black_both_ThruCheck1) {
    ...
    std::vector<std::string> expected_moves = {// Castling: Queenside OK (e8c8). Kingside Disallowed.
...
TEST(castle_test, allowed_Black_both_enemyclose) {
    ...
    std::vector<std::string> expected_moves = {"e8d8", "e8e7", "e8f8", "e8d7", "e8f7", "e8g8", "e8c8",
                                               // Rook a8
                                               "a8b8", "a8c8", "a8d8", "a8a7", "a8a6", "a8a5", "a8a4", "a8a3", "a8a2", "a8a1",
                                               // Rook h8
                                               "h8g8", "h8f8", "h8h7", "h8h6", "h8h5", "h8h4", "h8h3", "h8h2", "h8h1"};
...
TEST(castle_test, kingside_white) {
    ...
    std::array<Move, max_legal_moves> moves;
    size_t num_moves = board.get_moves<normal_search, true>(moves);
    std::vector<std::string> expected_moves = {"e1f1", "e1g1", "h1g1", "h1f1", "h2h3", "h2h4", "e1d1", "e1d2", "e1e2", "e1f2"};
    std::vector<std::string> generated_moves;
    for (size_t i = 0; i < num_moves; ++i) {
        generated_moves.push_back(moves[i].toString());
    }
...
TEST(Movegentest, three_deep) {
    ...
    ASSERT_EQ(expected, num_moves) << "Expected number of moves is 8902. Actual found moves is" << num_moves;
}
TEST(Movegentest, four_deep) {
    ...
    ASSERT_EQ(expected, num_moves) << "Expected number of moves is 197281. Actual found moves is" << num_moves;
}
