...
TEST(Movegentest, p3) {
    std::vector<int> moves = {14, 191, 2812, 43238};
...
TEST(Movegentest, p4) {
    std::vector<int> moves = {6, 264, 9467, 422333};
    ...
    starting_fen = "r2q1rk1/pP1p2pp/Q4n2/bbp1p3/Np6/1B3NBn/pPPP1PPP/R3K2R b KQ - 0 1";
    for (int i = 0; i < max; i++) {
        int num_moves = movegen_benchmark::gen_num_moves(starting_fen, i + 1);
        size_t expected = moves[i];
        ASSERT_EQ(expected, num_moves);
