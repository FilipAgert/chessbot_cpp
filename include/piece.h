...
struct Piece {
 ...
 public:
    ...
    constexpr explicit Piece(T v) {
        value = static_cast<uint8_t>(v);
    }

    constexpr Piece() : value(pieces::none) {}  // Default constructor initializes to none
    /**
     * @brief Creates and returns a piece given a character representing it. error piece if nonvalid
     * character.
     *
     * @param c
     * @return Piece
    ...
    inline static bool are_colors_equal(const Piece color1, const Piece color2) {
        return (color1.value & pieces::color_mask) == (color2.value & pieces::color_mask);
    }
    inline static bool are_types_equal(const Piece type1, const Piece type2) {
        return (type1.value & pieces::piece_mask) == (type2.value & pieces::piece_mask);
    }
    inline static bool is_valid_piece(const Piece p) {
        return (p.value & pieces::piece_mask) != pieces::none &&
               (p.value & pieces::color_mask) != pieces::none;
    }
    inline bool operator==(const Piece &other) const { return value == other.value; }
